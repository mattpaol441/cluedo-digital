...
export interface CluedoGameState { // Da verificare se va bene così
    ...
    // TIMEOUT SYSTEM (per gestione disconnessioni)
    turnStartedAt?: number;   // Timestamp (ms) di inizio turno
    stageStartedAt?: number;  // Timestamp (ms) di inizio stage refutation
}